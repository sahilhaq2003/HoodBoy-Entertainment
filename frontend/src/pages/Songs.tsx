    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
        {[
          ...
          <div key={i} className="bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md hover:border-gray-300 transition-all duration-200 p-4">
            ...
            <div className="h-1 rounded-full bg-gray-200 overflow-hidden mt-2"><div className="h-full rounded-full transition-all duration-700" style={{ width: `${(s.value / Math.max(songs.length, 1)) * 100}%`, background: s.color }} /></div>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg flex-1 min-w-48 bg-white border border-gray-200">
          ...
          <input type="text" placeholder="Search songs..." value={search} onChange={e => setSearch(e.target.value)} className="bg-transparent text-sm text-gray-700 placeholder-gray-400 outline-none flex-1" />
        </div>
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="px-3 py-2 bg-white border border-gray-300 rounded-lg text-sm text-gray-900 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/10">
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {songs.map(song => {
            ...
            return (
              <div key={song._id} className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden transition-all duration-200 hover:shadow-md">
                {/* Song row */}
                <div className="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-gray-50 transition-colors" onClick={() => loadDetail(song._id)}>
                  <div className="flex-shrink-0">
                    {isExpanded ? <ChevronDown size={16} className="text-gray-400" /> : <ChevronRight size={16} className="text-gray-400" />}
                  </div>
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-purple-50 border border-purple-200">
                    <Music size={14} className="text-purple-500" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-semibold text-gray-900">{song.title}</div>
                    <div className="text-xs text-gray-500">{song.artist?.stageName || song.artist?.name} {song.album ? `· ${song.album}` : ''}</div>
                  </div>
                  ...
                  <div className="hidden md:block text-xs text-gray-500 w-20 text-right">{formatNumber(song.streams || 0)} streams</div>
                  <div className="hidden md:block text-xs font-medium text-emerald-600 w-20 text-right">{song.revenue ? formatCurrency(song.revenue) : '—'}</div>
                </div>
                      ...
                      <div className="space-y-5">
                        ...
                        <div>
                          <div className="flex items-center justify-between gap-3 mb-3">
                            <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Production Workflow (12 Steps)</h4>
                            <button onClick={() => openEditSong(songDetail)} className="px-3 py-1.5 rounded-lg border border-gray-200 bg-white text-xs font-medium text-gray-600 hover:text-indigo-600 hover:border-indigo-200 flex items-center gap-1.5"><Edit2 size={12} /> Edit Song Details</button>
                            <button onClick={() => setSongDeleteTarget(song)} className="px-3 py-1.5 rounded-lg border border-red-200 bg-white text-xs font-medium text-red-600 hover:bg-red-50 hover:border-red-300 flex items-center gap-1.5"><Trash2 size={12} /> Delete</button>
                          </div>
                            ...
                            {(songDetail.productionWorkflow || []).map(step => (
                              ...
                              >
                                <div className="flex items-center gap-2">
                                  ...
                                  <select value={step.status} onChange={event => updateWorkflowStep(step._id, { status: event.target.value })} className="bg-white/80 border border-current/20 rounded px-1.5 py-1 text-[10px] text-gray-700 outline-none">
                                ...
                                }} placeholder="Notes or blocker reason" className="w-full mt-2 px-2 py-1.5 rounded border border-gray-200 bg-white text-[10px] text-gray-600 outline-none focus:border-indigo-300" />
                              </div>
                            ))}
                          </div>
                        </div>
                        ...
                        <div>
                          <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Song Versions</h4>
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                            {ALL_VERSIONS.map(vType => {
                              ...
                              return (
                                <div key={vType} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium border ${
                                  exists ? 'bg-green-50 border-green-200 text-green-700' : 'bg-white border-gray-200 text-gray-400'
                                }`}>
                                  {exists ? <CheckCircle2 size={14} className="text-green-500 flex-shrink-0" /> : <FileAudio size={14} className="text-gray-300 flex-shrink-0" />}
                                  <div className="min-w-0 flex-1">
                                    <div className={exists ? 'text-green-700' : 'text-gray-500'}>{VERSION_LABELS[vType]}</div>
                                  ...
                                  {exists ? (
                                    <div className="flex items-center gap-1">
                                      ...
                                      <button onClick={() => setVersionDeleteTarget(exists._id)} className="p-1 text-gray-400 hover:text-red-500" title="Remove"><Trash2 size={12} /></button>
                                    </div>
                                  ) : (
                                    <label className="px-2 py-1 rounded-md bg-indigo-50 text-indigo-600 hover:bg-indigo-100 cursor-pointer flex items-center gap-1">
                        ...
                        {songDetail.credits && songDetail.credits.length > 0 && (
                          <div>
                            <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Credits</h4>
                            <div className="flex flex-wrap gap-2">
                              {songDetail.credits.map((c, i) => (
                                <span key={i} className="px-2.5 py-1 rounded-lg bg-indigo-50 border border-indigo-200 text-xs font-medium text-indigo-700">
      ...
      {showSongForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-3xl max-h-[92vh] overflow-y-auto bg-white rounded-2xl border border-gray-200 shadow-2xl">
            <div className="sticky top-0 z-10 px-6 py-4 bg-white border-b border-gray-200 flex items-center justify-between">
              <div>
                <h3 className="text-lg font-bold text-gray-900">{editingSong ? 'Edit Song' : 'Create Song'}</h3>
                <p className="text-xs text-gray-500 mt-0.5">Song details, beat ownership, and production credits</p>
              </div>
              <button onClick={() => setShowSongForm(false)} className="p-1 text-gray-400 hover:text-gray-700"><X size={19} /></button>
            </div>

            <div className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5">Song Title <span className="text-red-500">*</span></label>
                  <input value={songForm.title} onChange={event => setSongForm({ ...songForm, title: event.target.value })} placeholder="Song title" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none focus:border-indigo-500" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5">Artist <span className="text-red-500">*</span></label>
                  <select value={songForm.artist} onChange={event => setSongForm({ ...songForm, artist: event.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white outline-none focus:border-indigo-500">
                    <option value="">Select artist...</option>
                    {artists.map(artist => <option key={artist._id} value={artist._id}>{artist.artistName || artist.stageName || artist.name}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5">Project / Album</label>
                  <input value={songForm.album} onChange={event => setSongForm({ ...songForm, album: event.target.value })} placeholder="Album, EP, or single project" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none focus:border-indigo-500" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5">Genre</label>
                  <input value={songForm.genre} onChange={event => setSongForm({ ...songForm, genre: event.target.value })} placeholder="Genre" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none focus:border-indigo-500" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5">Status</label>
                  <select value={songForm.status} onChange={event => setSongForm({ ...songForm, status: event.target.value as Song['status'] })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white outline-none focus:border-indigo-500">
                ...
                <div>
                  ...
                  <select value={songForm.priority} onChange={event => setSongForm({ ...songForm, priority: event.target.value as Song['priority'] })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white outline-none focus:border-indigo-500">
                    <option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="critical">Critical</option>
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5">Produced By</label>
                  <input value={songForm.producedBy} onChange={event => setSongForm({ ...songForm, producedBy: event.target.value })} placeholder="Producer credit display" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none focus:border-indigo-500" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5">Written By</label>
                  <input value={songForm.writtenBy} onChange={event => setSongForm({ ...songForm, writtenBy: event.target.value })} placeholder="Writer credit display" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none focus:border-indigo-500" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5">ISRC Code</label>
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5">Duration (seconds)</label>
                  <input type="number" min="0" value={songForm.duration} onChange={event => setSongForm({ ...songForm, duration: event.target.value })} placeholder="e.g. 240" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none focus:border-indigo-500" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5">Release Date</label>
                  <input type="date" value={songForm.releaseDate} onChange={event => setSongForm({ ...songForm, releaseDate: event.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none focus:border-indigo-500" />
                </div>
              </div>

              <div className="rounded-xl border border-amber-200 bg-amber-50/60 p-4">
                <h4 className="text-xs font-bold text-amber-800 uppercase tracking-wide mb-3">Beat Ownership</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <input value={songForm.beatProducer} onChange={event => setSongForm({ ...songForm, beatProducer: event.target.value })} placeholder="Beat producer" className="w-full px-3 py-2 bg-white border border-amber-200 rounded-lg text-sm outline-none focus:border-amber-500" />
                  <select value={songForm.beatLicenseType} onChange={event => setSongForm({ ...songForm, beatLicenseType: event.target.value })} className="w-full px-3 py-2 bg-white border border-amber-200 rounded-lg text-sm outline-none focus:border-amber-500">
                  ...
                  <div><label className="block text-[10px] text-amber-700 mb-1">Purchase date</label><input type="date" value={songForm.beatPurchaseDate} onChange={event => setSongForm({ ...songForm, beatPurchaseDate: event.target.value })} className="w-full px-3 py-2 bg-white border border-amber-200 rounded-lg text-sm outline-none focus:border-amber-500" /></div>
                ...
                <label className="mt-3 flex items-center gap-2 text-xs font-medium text-amber-900 cursor-pointer">
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <h4 className="text-xs font-bold text-gray-600 uppercase tracking-wide">Credits</h4>
                ...
                <div className="space-y-2">
                  {songForm.credits.map((credit, index) => (
                    <div key={index} className="grid grid-cols-1 sm:grid-cols-[1fr_150px_90px_1fr_auto] gap-2 p-3 bg-gray-50 border border-gray-200 rounded-lg">
                      <input value={credit.name} onChange={event => updateCredit(index, { name: event.target.value })} placeholder="Name" className="px-2 py-1.5 bg-white border border-gray-200 rounded text-xs outline-none focus:border-indigo-400" />
                      <select value={credit.role} onChange={event => updateCredit(index, { role: event.target.value as CreditRole })} className="px-2 py-1.5 bg-white border border-gray-200 rounded text-xs outline-none focus:border-indigo-400">
                      ...
                      <input value={credit.notes} onChange={event => updateCredit(index, { notes: event.target.value })} placeholder="Notes" className="px-2 py-1.5 bg-white border border-gray-200 rounded text-xs outline-none focus:border-indigo-400" />
                      <button type="button" disabled={songForm.credits.length === 1} onClick={() => setSongForm(current => ({ ...current, credits: current.credits.filter((_, i) => i !== index) }))} className="p-1.5 text-gray-400 hover:text-red-500 disabled:opacity-30"><X size={14} /></button>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1.5">Production Notes</label>
                <textarea value={songForm.notes} onChange={event => setSongForm({ ...songForm, notes: event.target.value })} rows={3} placeholder="Concept, references, recording instructions, or other notes" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none focus:border-indigo-500 resize-none" />
              </div>
            </div>

            <div className="sticky bottom-0 px-6 py-4 bg-white border-t border-gray-200 flex justify-end gap-3">
              <button onClick={() => setShowSongForm(false)} className="px-4 py-2 border border-gray-200 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50">Cancel</button>
